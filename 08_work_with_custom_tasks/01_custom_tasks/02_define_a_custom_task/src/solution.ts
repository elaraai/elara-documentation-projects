...
const custom_task = new CustomTaskBuilder("Quadratic")
    ...
    .input(
        ...
        // 2.4 Define the file as not executable 
        false,
        // 2.5 Define the expressions to serialize the input
        float => Utf8Encode(Print(float)),
    )
    // 2.6 Use a shell comaand to calculate the quadratic equation, and output the result to a file
    .shell(`awk '{x=$1; printf "%.14f\\n", x * (10 - x)}' input.txt > output.txt`)
    // 2.7 Add an output of the custom task
    .output(
        // 2.8 The filename of the output
        "output.txt",
