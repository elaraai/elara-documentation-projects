 ...
 * ## Steps taken:
 * 
 * 1. **Datasource Building**: Add patch data sources for suppliers, orders and price.
 * 2. **Scenario Building (predictive)**: Adapt the predictive scenario to alter suppliers, orders and the price from the patch data sources.
 * 3. **Layout Building**: Adapt the suppliers and orders layouts, to use the patch data sources with constraints and targets, create a new form layout to do the same for price.
 * 4. **Dashboard Building**: Adapt the headers to exclude the price value, and add the price form to the panel
 *
 */
