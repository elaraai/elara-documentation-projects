 ...
 * 
 * This TypeScript code defines a predictive simulation solution, by extending the descriptive solution.
 * 
 * ## Steps taken:
 * 
 * 1. **Pipeline Building**: Add a pipeline to calculate an end date for the simulation
 * 2. **Machine Learning (ML) Building**: Define ML models using the `MLModelBuilder` to infer demand and supplier choices, and train them on historic data.
 * 3. **Function Building**: Add logic to key dates functions to predict future dates for sales and procurement, extend to use ml to predict future orders.
 * 4. **Resource Building**: Define a new resource to model a schedule of future orders, mapped from the procurement dates function output.
 * 5. **Process Building**: Define processes for future sales and procurement, which can execute the existing processes.
 * 6. **Scenario Building (predictive)**: Create a predictive scenario to simulate future business outcomes, as a continuation of the past.
 * 7. **Pipeline Building**: Add a pipeline to concatenate the reports from both descriptive and predictive scenarios together.
 * 8. **Layout Building**: Build layouts to display orders and suppliers, and update charts to use the concatenated reports
 * 9. **Dashboard Building**: Construct a dashboard that combines the new layout into a panel, and uses predictive values for the header
 * 
 */
    ...
    .header(
        builder => builder
            ...
            .item(
                "Profit",
                builder => builder
                    .fromStream(predictive.simulationResultStreams().Cash)
                    .value((value) => PrintTruncatedCurrency(value),)
            )
            .item(
                "Inventory",
                builder => builder
                    .fromStream(predictive.simulationResultStreams().Inventory)
                    .value((value) => value)
            )
            .item(
                "Liability",
