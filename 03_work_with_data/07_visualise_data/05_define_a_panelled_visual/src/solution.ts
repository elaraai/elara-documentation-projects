...
// 2. Define a layout with a panel that contains a table and a vega chart that displays the source
const my_layout = new LayoutBuilder("My Layout")
    .panel('row', builder => builder
        // 2.1 Add a table to the panel with a size of 50%
        .table("50%", "My Table", builder => builder
            .fromStream(my_source.outputStream())
            .columns()
        )
        // 2.2 Add a vega chart to the panel with a size of 50%
