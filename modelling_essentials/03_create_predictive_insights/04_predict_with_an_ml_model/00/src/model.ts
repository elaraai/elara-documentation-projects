import { Convert, DateTimeType, FloatType, IntegerType, MLModelBuilder, MLPredictionBuilder, PipelineBuilder, Print, SourceBuilder, StringType, Template } from "@elaraai/core"
...
const demand_values = new SourceBuilder("Demand Values")
    .value({
        value: new Map(
            Array.from({ length: 20 }).map((_, index) => [`${index}`, { discount: index }])
        )
    });

const demand_validation = new MLPredictionBuilder("Demand Validation")
    .model(demand)
    .predict(demand_values.outputStream())
