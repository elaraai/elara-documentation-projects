import { ArrayType, BlobType, DateTimeType, DictType, FloatType, Floor, GetField, IfNull, IntegerType, LayoutBuilder, Multiply, Nullable, PipelineBuilder, SourceBuilder, Stream, StringJoin, StringType, StructType, Subtract, Sum, Template, Unique, } from "@elaraai/core"
...
const productRebate = new SourceBuilder("Rebate")
    .value({
        value: new Map(),
...
const summary_statistics_per_product_code = new PipelineBuilder("Summary Statistics Per Product Code")
    ...
    .input({ name: "productRebate", stream: productRebate.outputStream() })
    // .transform({
    //     //TODO: from here
    //     // apply rebate here,
    //     // and create rebate field
    //     // Then add editable column to table allowing edits to rebate datastream
    // })
    .select({
        keep_all: true,
        selections: {
